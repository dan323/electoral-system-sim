package com.dan323.utils.comparators;
...
public class RandomComparatorTest {
    ...
    public void testRepeatRandom() {
        ...
        for (int i = 0; i < 50; i++) {
            Assertions.assertTrue(x * randomComparator.compare("A", "B") > 0);
        }
    }
    ...
    public void checkCompareTransitive() {
        for (int i = 0; i < 50; i++) {
            ...
            if (x1 * x2 < 0) {
                Assertions.assertTrue(randomComparator.compare("A", "C") * x1 > 0);
            } else if (x2 * x3 < 0) {
                Assertions.assertTrue(randomComparator.compare("B", "D") * x2 < 0);
            }
    ...
    public void compareEquals() {
        Assertions.assertEquals(0, randomComparator.compare("A", "A"));
    }
    ...
    public void eraseOrder() {
        ...
        Assertions.assertTrue(map.get("pos") <= 60000);
        Assertions.assertTrue(map.get("pos") >= 40000);
        Assertions.assertTrue(map.get("neg") <= 60000);
        Assertions.assertTrue(map.get("neg") >= 40000);
